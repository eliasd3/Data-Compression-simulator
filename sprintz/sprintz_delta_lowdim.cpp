...
static const int kDefaultGroupSzBlocks = 1; //was 2
            ...
            for (uint16_t dim = 0; dim < ndims; dim++) {
                ...
                for (uint8_t i = 0; i < block_sz; i++) {
                    ...
                    if(thread!=NULL){
                        ...
                        thread->newValuePredicted((int) delta);
...
int64_t compress_rowmajor_delta_rle_lowdim_8b(const uint8_t* src, uint32_t len,
                                              int8_t* dest, uint16_t ndims, bool write_size,Thread* thread)
{
    return compress_rowmajor_delta_rle_lowdim(src, len, dest, ndims, write_size,NULL);
}
...
{
    return compress_rowmajor_delta_rle_lowdim(src, len, dest, ndims, write_size, thread);
}
