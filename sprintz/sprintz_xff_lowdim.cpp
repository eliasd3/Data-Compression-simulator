...
static const int kDefaultGroupSzBlocks = 1;// // // was 2 before
...
int64_t compress_rowmajor_xff_rle_lowdim(const uint_t* src, uint32_t len,
                                         int_t* dest, uint16_t ndims, bool write_size, Thread* thread)
{
        ...
        while (b < group_sz_blocks) { // for each block
            ...
            for (uint16_t dim = 0; dim < ndims; dim++) {
                ...
                for (uint8_t i = 0; i < block_sz; i++) {
                    ...
                    //TODOO: maken da je hier het object thread ebt, meegeven,
                    //      daarna vanuit hier functie oproepen uit die thread
                    //      die functie kan dan signal emitten om error up te daten en te printen op mainwindow!


                    if(thread!=NULL){
                        auto start_time2 = std::chrono::system_clock::now();

                        thread->newValuePredicted((int) err);

                        auto end_time2 = std::chrono::system_clock::now();
                        auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end_time2 - start_time2).count();
                        thread->setHowManySleepsFire(duration);
                    }
...
do_rle:
            ...
            b++;
            // qDebug() << "_";
            thread->fireCompressedDataUpdate(ngroups); //its weird that sometimes ngroups goes from 40-42 at once.

        } // for each block
            //TODOO: destination pointer hier met functie (van thread) doorsturen,
            //       iedere keer dat is doorgestuurd, graph updaten met destination[] uit te
            //       lezen zoals dak deed in main.
    } // for each group
...
{
    return compress_rowmajor_xff_rle_lowdim(src, len, dest, ndims, write_size, NULL);
}
int64_t compress_rowmajor_xff_rle_lowdim_16b(const uint16_t* src, uint32_t len,
                                             int16_t* dest, uint16_t ndims, bool write_size, Thread* thread)
{
    return compress_rowmajor_xff_rle_lowdim(src, len, dest, ndims, write_size, thread);
}
